...
type Tab = 'home' | 'path' | 'presets' | 'progress' | 'profile';

const tabs: { id: Tab; emoji: string; label: string }[] = [
  { id: 'home', emoji: '🏠', label: 'Главная' },
  { id: 'path', emoji: '🗺️', label: 'Путь' },
  { id: 'presets', emoji: '📸', label: 'Фото' },
...
export default function App() {
    ...
    return (
      <SafeAreaProvider>
        ...
        <PhotoSessionScreen
          ...
          onComplete={() => setActivePresetId(null)}
          onExit={() => setActivePresetId(null)}
        />
      </SafeAreaProvider>
    );
  }
  ...
  const renderScreen = () => {
    switch (activeTab) {
      ...
      case 'presets':
        return <PresetsScreen onSelectPreset={(id) => setActivePresetId(id)} />;
      case 'progress':
